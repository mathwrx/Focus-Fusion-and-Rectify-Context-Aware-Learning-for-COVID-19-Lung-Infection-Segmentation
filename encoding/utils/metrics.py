...
def batch_sores(predict, target, image):
    ...
    for i in range(batch_size):
        ...
        pixACC = (TP + TN) / float(TP + TN + FP + FN + 1e-9)
        batch_acc += pixACC

        # Compute Dice coefficient
        intersection = np.logical_and(target[i], refine_pred)
        dice = 2. * intersection.sum() / (refine_pred.sum() + target[i].sum() + 1e-9)
        batch_dice += dice

        # Compute Jaccard similarity coefficient score
        jacc = TP / float(TP + FN + FP + 1e-9)
        batch_jacc += jacc

        # Compute the Sensitivity (recall)
        sensitivity = TP / float(TP + FN + 1e-9)
        batch_sensitivity += sensitivity

        # Compute Specificity score
        specificity = TN / float(TN + FP + 1e-9)
